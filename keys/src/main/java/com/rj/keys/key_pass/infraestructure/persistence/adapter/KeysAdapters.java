...
public class KeysAdapters implements Keys_RepositoryPort {
    ...
    public keys_pass_Entity SetKey() {
        List<keys_pass_Entity> existsKeys = new ArrayList<>();
        try {
            ...
            if (existsKeys.isEmpty()) {
                ...
                keys_pass_Entity oldKey = existsKeys.stream().filter(k -> k.getId() == 2).findFirst().orElse(existsKeys.get(1));
                repository.delete(oldKey);

                keys_pass_Entity currentKey = existsKeys.stream().filter(k -> k.getId() == 1).findFirst().orElse(existsKeys.get(0));
    ...
    private keys_pass_Entity saveNewKey(int id, String pub, String priv, boolean state, LocalDateTime now) {
        keys_pass_Entity newKey = new keys_pass_Entity();
        newKey.setId(id);
    ...
    private void rotateKey(keys_pass_Entity key, int newId, boolean newState, LocalDateTime now) {
